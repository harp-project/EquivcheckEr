...
run_tests(Functions, OrigNode, RefacNode, Types, CallGraph, Results) ->
    ...
    % A result is a tuple: {FileName, MFA, Counterexample}
    ...
    Callers = lists:uniq(lists:flatmap(fun({FileName, {_,F,A}, _}) -> CallGraph({FileName, F, A}, refactored) end, FailedFuns)),
...
% Gets a module and a list of types, and gives back a PropEr type for creating the generator
-spec convert_type(atom(), [type()]) -> 
    [proper_types:rich_result(proper_types:fin_type())].
convert_type(M, ArgTypes) ->
    lists:map(fun(ArgType) ->
                      element(2, proper_typeserver:translate_type({M, ArgType})) end,
              ArgTypes).
